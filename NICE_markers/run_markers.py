...
def process_subject_task_pair(args):
    ...
    derivatives_folder, subject, task, output_dir, modes = args
    return process_file(derivatives_folder, subject, task, output_dir, modes)

def main():
    parser = argparse.ArgumentParser(description="Run NICE marker analysis for all subjects/tasks.")
    ...
    for subject in subjects:
        ...
        for task in tasks:
            subject_task_pairs.append((derivatives_folder, subject, task, output_dir, modes))
    ...
    if slurm_array_task_id is not None and slurm_ntasks is not None:
        ...
        pairs_per_task = (task_pairs_count + ntasks - 1) // ntasks
