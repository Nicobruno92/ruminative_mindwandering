...
muscle_components, muscle_scores = ica.find_bads_muscle(epochs_clean
, threshold=0.7)
...
ic_labels = label_components(epochs_clean
, ica, method="iclabel")
...
# Automatically reject bad epochs using AutoReject
ar = AutoReject(thresh_method="bayesian_optimization", cv = folds, random_state=42, n_jobs = -1, )
epochs_interpolated_clean = ar.fit_transform(epochs_interpolated)
reject = get_rejection_threshold(epochs_interpolated)

# Log the epochs rejected by AutoReject
ar_reject_epochs = [
    n_epoch
    for n_epoch, log in enumerate(epochs_interpolated_clean.drop_log)
    if log == ("AUTOREJECT",)
]
...
# Add the cleaned epochs to the report
report.add_epochs(epochs=epochs_interpolated_clean, title="Epochs clean after ICA", psd=False)

# Save the cleaned epochs
epochs_interpolated_clean.drop_bad()

# Log the epochs dropped by ICA
log_preprocessing.log_detail("epochs_drop_log", epochs_interpolated_clean.drop_log)
log_preprocessing.log_detail("epochs_drop_log_description", epochs_interpolated_clean.drop_log)
...
p300_evoked = mne.combine_evoked([epochs_interpolated_clean['go/correct'].average(), epochs_interpolated_clean['nogo/correct'].average()], weights = [1,-1])
report.add_evokeds(
