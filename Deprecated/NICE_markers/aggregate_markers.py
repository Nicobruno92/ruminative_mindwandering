        ...
        if len(df) > chunk_size:
            ...
            for i in range(num_chunks):
                ...
                if valid_data.any():
                    ...
                    # Aggregate this chunk
                    chunk_agg = chunk.groupby(groupby_cols, observed=True, sort=False)['mean'].agg(
                        mean='mean',
                        std=lambda x: np.std(x, ddof=1) if len(x) > 1 else 0,
                        count='count'
                    ).reset_index()
                    # Add raw condition values if needed
                    for condition in conditions:
                        if condition in ['trial_type', 'correctness']:
                            continue
                        if condition in chunk.columns:
                            chunk_agg[condition] = chunk.groupby(groupby_cols, observed=True, sort=False)[condition].mean().values
                    chunk_results.append(chunk_agg)
                # Free memory
                del chunk
                gc.collect()
            ...
            if chunk_results:
                ...
                print("Re-aggregating results across chunks...")
                agg_df = agg_df.groupby(groupby_cols, observed=True, sort=False).agg(agg_dict).reset_index()
            else:
                agg_df = None
        else:
            # Small enough to process all at once
            ...
            if valid_data.any():
                ...
                for condition in conditions:
                    ...
                    if condition in df_filtered.columns:
                        agg_df[condition] = df_filtered.groupby(groupby_cols, observed=True, sort=False)[condition].mean().values
            else:
                agg_df = None
    except Exception as e:
        print(f"Error during aggregation: {e}")
    ...
    try:
        ...
        if len(task_df) > 100000:
            ...
            for subject in task_df['subject_id'].unique():
                ...
                subject_agg = subject_data.groupby(groupby_cols, observed=True, sort=False)['mean'].agg(
                    mean='mean',
                    std=lambda x: np.std(x, ddof=1) if len(x) > 1 else 0,
                    count='count'
                ).reset_index()
                ...
                for condition in conditions:
                    ...
                    if condition in subject_data.columns:
                        subject_agg[condition] = subject_data.groupby(groupby_cols, observed=True, sort=False)[condition].mean().values
            ...
            del chunks
            gc.collect()
            # Re-aggregate in case of split groups
            subject_df = subject_df.groupby(groupby_cols, observed=True, sort=False).agg(agg_dict).reset_index()
        else:
            ...
            for condition in conditions:
                if condition in ['trial_type', 'correctness']:
                    continue
                if condition in task_df.columns:
                    subject_df[condition] = task_df.groupby(groupby_cols, observed=True, sort=False)[condition].mean().values
    except Exception as e:
