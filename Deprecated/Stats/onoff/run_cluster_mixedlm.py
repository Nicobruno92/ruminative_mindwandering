...
# -*- coding: utf-8 -*-
"""
Script para ejecutar el análisis de cluster mixedlm para un marcador específico.
Este script se ejecuta desde run_cluster_mixedlm_slurm.sh como parte de un trabajo en array.
"""
...
N_PERM = 500          # Number of permutations for robust statistics
...
MIN_PERMUTATIONS = 200  # Minimum before early stopping
EARLY_STOP_THRESHOLD = 0.001  # Stop early if p-value precision is sufficient
MAX_PERMUTATIONS = 2000  # Maximum permutations
...
MIN_CHANNELS_FOR_CLUSTER = 4  # Minimum cluster size
...
def compute_lmm_tmap(df_marker: pd.DataFrame, ch_names, use_zscore=True, rng=None):
    ...
    for i, ch in valid_channels:
        ...
        try:
            ...
            model = mixedlm(f'{dv} ~ cond', d_ch, groups=d_ch['subject_id'], re_formula=f'~cond')
            res = model.fit(
                method='lbfgs', 
                reml=False, 
                maxiter=100,
...
def run_mixedlm_cluster_test(df, marker, n_perm=N_PERM, alpha=ALPHA, rng=None):
    ...
    # ------------------------------------------------------------------
    # null distribution – permute labels within subject
    # ------------------------------------------------------------------
    ...
    return {
        ...
        'adaptive_testing': False,  # This script doesn't use adaptive testing
...
def process_marker(marker, n_perm=N_PERM, alpha=ALPHA, rng=None):
    """Process a single marker and save results."""
    os.makedirs(OUT_DIR, exist_ok=True)
    
    print(f"Loading data for marker: {marker}")
    ...
    for chunk in tqdm(chunks, desc="Processing chunks"):
        # Filter chunk by marker
        filtered_chunk = chunk[chunk['marker'] == marker]
    ...
    save_analysis_parameters(
        ...
        adaptive_testing=res.get('adaptive_testing', False),
    ...
    # ------------------------------------------------------------------
    # Create plot if plotting function is available
    # ------------------------------------------------------------------
    print(f"   🎨 Starting plot creation for marker {marker}...")
    ...
    if plot_cluster_results:
        print("   📊 Using original plot_cluster_results function")
        ...
        print("   🏗️ Using built-in plot_cluster_results function")
    ...
    print(f"{marker:>10}: {sig} significant cluster(s) (α={alpha})")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run improved mixed linear model cluster permutation test for a specific marker')
    parser.add_argument('--marker', required=True, help='Marker to analyze')
    parser.add_argument('--permutations', type=int, default=N_PERM, help='Number of permutations')
    ...
    # Process the specified marker
    process_marker(args.marker, n_perm=args.permutations, alpha=args.alpha, rng=rng)
