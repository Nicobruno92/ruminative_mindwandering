#!/usr/bin/env python3
"""
LMM Interaction Analysis with FDR Correction - OnOff * Group

This script performs Linear Mixed-Effects Model (LMM) analysis with False Discovery Rate (FDR)
correction to examine the interaction between onoff conditions (on-task vs off-task) and groups
(Group 1 vs Group 2) in depressed patients.

The model: marker ~ onoff * group
This gives us three main effects:
1. Main effect of onoff (on-task vs off-task)
2. Main effect of group (Group 1 vs Group 2)  
3. Interaction effect (onoff * group)

Features:
- Loads probe-level aggregated data (both ON and OFF conditions)
- Merges with metadata to get group information
- Tests interaction between onoff and group
...
CSV_FILE = os.path.join(
    'results/aggregated_mne_markers/aggregated_mne_markers_onoff_5trials_go_correct_outliers_removed_iqr_probe.csv'
)

# Metadata file with group information
METADATA_FILE = os.path.join(project_root, 'metadata_experiment.csv')

OUT_DIR = os.path.join(project_root, 'results/onoff_group_interaction_analysis')

# LMM Formula configuration - interaction model
BASE_FORMULA = 'mean ~ onoff_label * group_label'  # Full interaction model
...
def filter_and_merge_data(df, subject_to_group):
    """
    Filter data and merge with group information (keep both ON and OFF conditions).
    ...
    df_filtered['group_label'] = df_filtered['group'].astype(int).astype(str)
    df_filtered['group_label'] = df_filtered['group_label'].astype('category')
    ...
    print(df_filtered['group_label'].value_counts().sort_index())
    print(f"OnOff x Group crosstab:")
    print(pd.crosstab(df_filtered['onoff_label'], df_filtered['group_label'], margins=True))
...
def compute_lmm_stats(df_marker: pd.DataFrame, ch_names, formula=BASE_FORMULA, group_col=RANDOM_EFFECTS_GROUP):
    ...
    dict
        ...
        - group_effect: main effect of group  
        - interaction_effect: onoff * group interaction
    """
    ...
    for i, ch in enumerate(ch_names):
        ...
        onoff_groups = d_ch['onoff_label'].nunique()
        groups = d_ch['group_label'].nunique()
        
        if onoff_groups < 2 or groups < 2:
            print(f"    Warning: Channel {ch} missing conditions (onoff: {onoff_groups}, groups: {groups})")
            for effect in results.values():
                effect['t_values'][i] = np.nan
                effect['p_values'][i] = np.nan
                effect['coef_values'][i] = np.nan
        ...
        try:
            ...
            # Main effect of group (2 vs 1)
            group_param = [p for p in param_names if 'group_label' in p and ':' not in p]
            if group_param:
                param_name = group_param[0]
                results['group_effect']['t_values'][i] = res.tvalues[param_name]
                results['group_effect']['p_values'][i] = res.pvalues[param_name]
                results['group_effect']['coef_values'][i] = res.params[param_name]
            else:
                results['group_effect']['t_values'][i] = np.nan
                results['group_effect']['p_values'][i] = np.nan
                results['group_effect']['coef_values'][i] = np.nan
            
            # Interaction effect
            interaction_param = [p for p in param_names if ':' in p and 'onoff_label' in p and 'group_label' in p]
...
def run_interaction_analysis(df, marker, formula=BASE_FORMULA, group_col=RANDOM_EFFECTS_GROUP, 
    ...
    condition_dist = pd.crosstab(df_m['onoff_label'], df_m['group_label'], margins=True)
    print(f"   Condition distribution:")
    print(condition_dist)
    
    if len(df_m['onoff_label'].unique()) < 2 or len(df_m['group_label'].unique()) < 2:
        print("   Warning: Less than 2 levels in onoff or group - skipping marker")
        return None
...
def plot_interaction_results(results, save_path=None, figsize=(20, 15)):
    ...
    fig.suptitle(f'Interaction Analysis: {marker}\n'
                 f'OnOff * Group Effects (FDR α = {fdr_alpha})', 
                 fontsize=18, fontweight='bold')
    
    effect_names = ['onoff_effect', 'group_effect', 'interaction_effect']
    effect_titles = ['Main Effect: OnOff (High vs Low)', 'Main Effect: Group (2 vs 1)', 'Interaction: OnOff × Group']
...
def main():
    print("🧠 INTERACTION ANALYSIS - ONOFF * GROUP (DEPRESSED PATIENTS) 🧠")
    ...
    if file_size_mb > 500:
        ...
        for chunk in tqdm(chunks, desc="Processing chunks"):
            # Filter for subjects with group info
            chunk_filtered = chunk[chunk['subject_id'].isin(subject_to_group.keys())]
    ...
    print(f"  Model: OnOff * Group interaction")
    ...
    for marker in markers_to_process:
        try:
            ...
            # Save results
            results_file = os.path.join(OUT_DIR, f"{marker}_interaction_results.csv")
            n_sig_saved = save_interaction_results(results, results_file)
            
            # Create plot
            plot_path = os.path.join(OUT_DIR, f"{marker}_interaction_plot.png")
    ...
    for results in all_results:
        ...
        for effect_name, effect_data in results['effects'].items():
            ...
            print(f"  {effect_name:>20}: {sig:>3} significant channel(s)")
    
    print("-" * 60)
    print(f"{'Total markers analyzed:':>25} {len(all_results)}")
    print(f"{'Model:':>25} OnOff * Group interaction")
