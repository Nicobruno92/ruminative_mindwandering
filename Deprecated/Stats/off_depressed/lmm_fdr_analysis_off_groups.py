#!/usr/bin/env python3
"""
LMM Analysis with FDR Correction - OFF Task Conditions Group Comparison

This script performs Linear Mixed-Effects Model (LMM) analysis with False Discovery Rate (FDR)
correction to compare OFF task conditions between group 1 and group 2 (depressed patients).
It uses the group information from metadata_experiment.csv to identify which subjects belong
to which group.

Features:
- Loads probe-level aggregated data filtered for OFF conditions only
- Merges with metadata to get group information
- Compares group 1 vs group 2 for OFF task conditions
- FDR correction for multiple comparisons
- Same plotting layout as original cluster analysis
"""
...
CSV_FILE = os.path.join(project_root, 'results/aggregated_mne_markers/aggregated_mne_markers_onoff_5trials_go_correct_outliers_removed_iqr_probe.csv')

# Metadata file with group information
METADATA_FILE = os.path.join(project_root, 'metadata_experiment.csv')

OUT_DIR = os.path.join(project_root, 'results/off_depressed_group_comparison')

# LMM Formula configuration - comparing groups for OFF conditions
BASE_FORMULA = 'mean ~ group_label'  # Compare group 1 vs group 2
...
def load_metadata(metadata_file):
    """
    Load metadata and create group mapping.
    
    Returns
    -------
    dict
        Dictionary mapping subject_id to group
    """
    ...
    print(f"Group 1 subjects: {sum(1 for g in subject_to_group.values() if g == 1)}")
    print(f"Group 2 subjects: {sum(1 for g in subject_to_group.values() if g == 2)}")
    
    return subject_to_group


def filter_and_merge_data(df, subject_to_group):
    """
    Filter data for OFF conditions only and merge with group information.
    ...
    subject_to_group : dict
        Mapping of subject_id to group
    ...
    """
    print("Filtering data for OFF conditions and merging with group info...")
    ...
    df_off['group'] = df_off['subject_id'].map(subject_to_group)
    
    # Remove subjects without group information
    before_filter = df_off.shape[0]
    df_off = df_off.dropna(subset=['group'])
    after_filter = df_off.shape[0]
    
    if before_filter != after_filter:
        print(f"Removed {before_filter - after_filter} rows due to missing group info")
    
    # Convert group to categorical for LMM
    df_off['group_label'] = df_off['group'].astype(int).astype(str)
    df_off['group_label'] = df_off['group_label'].astype('category')
    
    print(f"Final dataset: {df_off.shape[0]} rows")
    print(f"Group distribution:")
    print(df_off['group_label'].value_counts().sort_index())
...
def compute_lmm_stats(df_marker: pd.DataFrame, ch_names, formula=BASE_FORMULA, group_col=RANDOM_EFFECTS_GROUP):
    ...
    for i, ch in enumerate(ch_names):
        ...
        # Ensure we have both groups for this channel
        groups_present = d_ch['group_label'].nunique()
        if groups_present < 2:
            print(f"    Warning: Only {groups_present} group(s) present for channel {ch}")
            t_vec[i] = np.nan
            p_vec[i] = np.nan
            coef_vec[i] = np.nan
            continue
        ...
        try:
            ...
            # Get the parameter of interest (group comparison)
...
def run_lmm_fdr_analysis(df, marker, formula=BASE_FORMULA, group_col=RANDOM_EFFECTS_GROUP, 
    ...
    # Check group distribution for this marker
    group_dist = df_m['group_label'].value_counts().sort_index()
    print(f"   Group distribution:")
    for group, count in group_dist.items():
        print(f"     Group {group}: {count} observations")
    
    if len(group_dist) < 2:
        print("   Warning: Less than 2 groups present - skipping marker")
        return None
...
def plot_lmm_results(results, condition_high='Group 2', condition_low='Group 1', 
    ...
    # Main title
    fig.suptitle(f'LMM Analysis - OFF Task Group Comparison: {marker}\n'
    ...
    try:
        ...
        if len(available_channels) > 0:
            ...
            # Calculate group means for available channels
            df_m = results['df_marker']
            means = df_m.groupby(['group_label', 'channel'])['mean'].mean().unstack(level=0)
            
            if '1' in means.columns and '2' in means.columns:
                group1_filtered = np.array([means.loc[ch, '1'] for ch in available_channels if ch in means.index])
                group2_filtered = np.array([means.loc[ch, '2'] for ch in available_channels if ch in means.index])
                diff_filtered = group2_filtered - group1_filtered
            else:
                group1_filtered = group2_filtered = diff_filtered = None
            
            # First row: Raw values - group1, group2, and difference
            if group1_filtered is not None and group2_filtered is not None:
                # Plot group 1
                ax_group1 = fig.add_subplot(gs[0, 0])
                im_group1, _ = mne.viz.plot_topomap(group1_filtered, info, show=False, axes=ax_group1,
                                                   cmap='viridis', contours=6, sensors=True,
                                                   names=available_channels, outlines='head')
                ax_group1.set_title(f'{condition_low} (OFF-task) Raw Values')
                cbar_group1 = plt.colorbar(im_group1, ax=ax_group1, shrink=0.8)
                cbar_group1.set_label(f'{marker} value')
                
                # Plot group 2
                ax_group2 = fig.add_subplot(gs[0, 1])
                im_group2, _ = mne.viz.plot_topomap(group2_filtered, info, show=False, axes=ax_group2,
                                                   cmap='viridis', contours=6, sensors=True,
                                                   names=available_channels, outlines='head')
                ax_group2.set_title(f'{condition_high} (OFF-task) Raw Values')
                cbar_group2 = plt.colorbar(im_group2, ax=ax_group2, shrink=0.8)
                cbar_group2.set_label(f'{marker} value')
                ...
                legend_elements = [
                    Patch(facecolor='red', label='Group 2 > Group 1'),
                    Patch(facecolor='blue', label='Group 1 > Group 2')
                ]
            ...
            im_t, _ = mne.viz.plot_topomap(t_values_filtered, info, show=False, axes=ax_t,
                                          cmap='RdBu_r', contours=6, sensors=True,
                                          names=available_channels, outlines='head')
            ax_t.set_title('T-statistics (LMM) - Group Comparison', fontsize=14, fontweight='bold')
        ...
        if 'df_marker' in results:
            ...
            if '1' in means.columns and '2' in means.columns:
                group1_values = means['1'].reindex(ch_names).values
                group2_values = means['2'].reindex(ch_names).values
                
                ax_group1 = fig.add_subplot(gs[0, 0])
                ax_group1.bar(range(len(group1_values)), group1_values, color='blue')
                ax_group1.set_title(f'{condition_low} (OFF-task) Raw Values')
                ax_group1.set_xlabel('Channel Index')
                ax_group1.set_ylabel(f'{marker} value')
                
                ax_group2 = fig.add_subplot(gs[0, 1])
                ax_group2.bar(range(len(group2_values)), group2_values, color='red')
                ax_group2.set_title(f'{condition_high} (OFF-task) Raw Values')
                ax_group2.set_xlabel('Channel Index')
                ax_group2.set_ylabel(f'{marker} value')
                
                ax_diff = fig.add_subplot(gs[0, 2])
                diff_values = group2_values - group1_values
        ...
        bars = ax_t.bar(range(len(t_values)), t_values, 
                       color=['red' if t > 0 else 'blue' for t in t_values])
    ...
    if results['n_significant'] > 0:
        ...
        table.scale(1, 1.5)
        ax_stats.set_title(f'Top Significant Channels - Group Comparison (FDR α = {fdr_alpha})', pad=20)
        ...
        ax_stats.set_title('LMM Results Summary - Group Comparison')
        ax_stats.axis('off')
...
def main():
    print("🧠 LMM ANALYSIS - OFF TASK GROUP COMPARISON (DEPRESSED PATIENTS) 🧠")
    ...
    subject_to_group = load_metadata(METADATA_FILE)
    ...
    if file_size_mb > 500:
        ...
        for chunk in tqdm(chunks, desc="Processing chunks"):
            # Filter for OFF conditions and subjects with group info
            chunk_off = chunk[chunk['onoff_label'] == 'low']
            chunk_off = chunk_off[chunk_off['subject_id'].isin(subject_to_group.keys())]
    ...
    df_filtered = filter_and_merge_data(df, subject_to_group)
    ...
    print(f"Available groups: {sorted(df_filtered['group_label'].unique())}")
    ...
    print(f"  Comparison: Group 1 vs Group 2 (OFF task conditions only)")
    ...
    for marker in markers_to_process:
        try:
            ...
            # Save results
            results_file = os.path.join(OUT_DIR, f"{marker}_group_comparison_results.csv")
            n_sig_saved = save_lmm_results(results, results_file)
            
            # Create plot
            plot_path = os.path.join(OUT_DIR, f"{marker}_group_comparison_plot.png")
            
            try:
                fig = plot_lmm_results(
                    results,
                    condition_high='Group 2',
                    condition_low='Group 1',
                    save_path=plot_path
                )
                plt.close(fig)
                ...
                try:
                    ...
                    plt.title(f"T-statistics for {marker} (Group Comparison)")
                    ...
                    plt.text(0.1, 0.6, f"Comparison: Group 1 vs Group 2 (OFF task)", fontsize=10)
    ...
    # Summary
    print("\n" + "="*60)
    print("ANALYSIS SUMMARY - GROUP COMPARISON (OFF TASK)")
    print("="*60)
    ...
    print(f"{'Comparison:':>25} Group 1 vs Group 2 (OFF task only)")
