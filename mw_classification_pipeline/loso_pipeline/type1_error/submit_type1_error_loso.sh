#!/usr/bin/env bash
# =============================================================================
# SLURM Array Submission — Type I Error Evaluation (LOSO)
#
...
#   bash submit_type1_error_loso.sh
#
#   # Aggregate results after jobs finish:
#   bash submit_type1_error_loso.sh --aggregate
#
# CONFIGURATION:
...
PYTHON_SCRIPT="${SCRIPT_DIR}/run_type1_error_loso.py"

# Read parameters from configs via python (avoids duplicating YAML parsing in bash)
...
import yaml
c = yaml.safe_load(open('${PARENT_CONFIG}'))
v = c.get('model_type', ['rf'])
print(' '.join(v if isinstance(v, list) else [v]))
")

# SLURM resource settings (from parent config slurm block)
SLURM_TIME=$(python3 -c "import yaml; c=yaml.safe_load(open('${PARENT_CONFIG}')); print(c.get('slurm',{}).get('time','08:00:00'))")
SLURM_MEM=$(python3 -c "import yaml; c=yaml.safe_load(open('${PARENT_CONFIG}')); print(c.get('slurm',{}).get('mem','32G'))")
SLURM_CPUS=$(python3 -c "import yaml; c=yaml.safe_load(open('${PARENT_CONFIG}')); print(c.get('slurm',{}).get('cpus_per_task','8'))")
...
# ── Aggregate mode ─────────────────────────────────────────────────────────
if [[ "${1:-}" == "--aggregate" ]]; then
    echo "Aggregating Type I error results..."
...
echo "Submitting Type I error simulations (LOSO)"
...
for CONTRAST in $CONTRASTS; do
    for FAMILY in $FAMILIES; do
        for MODEL in $MODELS; do
            JOB_NAME="t1e_loso_${CONTRAST:0:8}_${FAMILY:0:6}_${MODEL}"
...
echo "  bash submit_type1_error_loso.sh --aggregate"
