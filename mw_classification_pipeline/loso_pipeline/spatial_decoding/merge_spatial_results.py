...
Reads the true per-channel AUCs (true/) and the permutation per-channel AUCs (perms/),
forms the max-over-channels null per permutation, computes per-channel family-wise
(FWER) p-values via the max-statistic test, and renders the AUC topomaps.
"""
...
def load_true_aucs(results_dir: str) -> dict:
    """Load per-channel true AUC from true/*.csv (shards or the combined file)."""
...
def build_max_null(results_dir: str) -> np.ndarray:
    ...
    for f in perm_files:
        p = pd.read_csv(f)
        max_per_perm.append(float(np.nanmax(p["auc"].to_numpy(dtype=float))))
    return np.asarray(max_per_perm, dtype=float)
...
def main():
    ...
    # Carry n_features / std_auc through from the true table.
