# %%
"""
Partial Correlation Analysis Between Probe Dimensions, PCA Components, and Psychometric Scales

This script creates a comprehensive partial correlation heatmap where:
- For probe dimensions: controls for the other 4 probe dimensions
- For PCA components:
  * With probe dimensions: controls for other probe dimensions
  * With psychometric scales: controls for all probe dimensions

Uses Pingouin for partial correlation calculations with FDR correction applied row-wise.
"""
...
OUTPUT_DIR = "results/Behavior/probe_data/partial_correlation_analysis"
...
def get_control_variables(row_var, col_var, available_scales):
    ...
    """
    # For probe dimensions as row variable:
    if row_var in PROBE_DIMENSIONS:
        # Control for other 4 probe dimensions
        control_vars = [dim for dim in PROBE_DIMENSIONS 
                       if dim != row_var and dim != col_var]
        
        # If column is a psychometric scale AND mutual control is enabled:
        # Also control for other psychometric scales
        if CONTROL_PSYCHOMETRICS_MUTUALLY and col_var in available_scales:
    ...
    # For PCA components as row variable:
    elif row_var in PCA_COMPONENTS:
        # If column is a probe dimension: control for other probe dimensions
        if col_var in PROBE_DIMENSIONS:
            control_vars = [dim for dim in PROBE_DIMENSIONS if dim != col_var]
        # If column is a psychometric scale: control for all probe dimensions
        else:
...
def create_partial_correlation_heatmap(corr_df, p_df, p_fdr_df, n_df, results_df):
    ...
    # Add title
    title_text = ('Partial Correlation Matrix: Probe Dimensions & PCA Components '
                 'vs Probe Dimensions & Psychometric Scales')
    ax.set_title(title_text, fontsize=18, fontweight='bold', pad=30)
    
    # Add subtitle with sample info
    n_subjects = n_df.max().max()
    subtitle_text = (f'N = {n_subjects:.0f} subjects | '
                    f'White cells = non-significant ({correction_label} p ≥ 0.05)')
    plt.figtext(0.5, 0.94, subtitle_text, ha='center', fontsize=12, style='italic')
    
    # Add control variable legend
    control_info = ("Controls: Probe dimensions → other probe dimensions | "
                   "PCA vs probes → other probe dimensions | "
                   "PCA vs psychometric → all probe dimensions")
    ...
    legend_text = (f"Significance levels ({correction_label} p-values): "
                  ...
                  "Color intensity = partial correlation strength")
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'partial_correlation_heatmap.png'), 
               dpi=DPI, bbox_inches='tight')
    plt.savefig(os.path.join(OUTPUT_DIR, 'partial_correlation_heatmap.svg'), 
               dpi=DPI, bbox_inches='tight')
...
def generate_partial_correlation_summary(results_df, df):
    ...
    """
    print("\\n" + "="*80)
    print("PARTIAL CORRELATION ANALYSIS SUMMARY REPORT")
    print("="*80)
    ...
    print(f"- Probe dimensions → Control for other 4 probe dimensions")
    if CONTROL_PSYCHOMETRICS_MUTUALLY:
        print(f"- Probe vs psychometric scales → Control for other probes AND other psychometric scales")
    else:
        print(f"- Probe vs psychometric scales → Control for other probes only")
    print(f"- PCA vs probe dimensions → Control for other probe dimensions")
    if CONTROL_PSYCHOMETRICS_MUTUALLY:
        print(f"- PCA vs psychometric scales → Control for all probes AND other psychometric scales")
    else:
        print(f"- PCA vs psychometric scales → Control for all probe dimensions only")
    ...
    results_file = os.path.join(OUTPUT_DIR, 'partial_correlation_results.csv')
...
def main():
    ...
    """
    print("="*80)
    print("PARTIAL CORRELATION ANALYSIS: PROBE DIMENSIONS & PSYCHOMETRICS")
    print("="*80)
