...
def plot_individual_delayed_mood_mediation_figures(results: pd.DataFrame, output_dir: str) -> None:
    """
    Create individual figures for each thought dimension.
    """
    print("\n" + "=" * 70)
    print("CREATING INDIVIDUAL DELAYED MOOD MEDIATION FIGURES")
    print("=" * 70)
    ...
    for dim in THOUGHT_DIMENSIONS:
        ...
        ax_forest.set_title(
            f"Moderated Mediation via {dim.upper()}: Condition → {dim.upper()} → Delayed Mood\n"
        ...
        # Path A is the same for all moods since it's Condition -> Thought
        row = dim_data.iloc[0]
        a_base = row["a_base"]
        a_base_se = row["a_base_se"]
        sig_base = abs(a_base / a_base_se) > 1.96
        
        ax_path_a.plot([a_base - 1.96*a_base_se, a_base + 1.96*a_base_se], [-0.15, -0.15],
                      color=COLOR_CONTROL, lw=2, alpha=1.0 if sig_base else 0.4,
                      linestyle='-' if sig_base else ':')
        ax_path_a.plot(a_base, -0.15, 'o', color=COLOR_CONTROL, markersize=12,
                      markerfacecolor=COLOR_CONTROL if sig_base else 'white',
                      markeredgewidth=2, alpha=1.0 if sig_base else 0.4)
        ax_path_a.text(a_base, -0.15 + 0.1, f"β={a_base:.2f}", ha='center', va='bottom',
                       fontsize=12, color=COLOR_CONTROL, fontweight='bold', alpha=1.0 if sig_base else 0.4)
        ...
        ax_path_a.plot([a_risk - 1.96*a_risk_se, a_risk + 1.96*a_risk_se], [+0.15, +0.15],
                      color=COLOR_RISK, lw=2, alpha=1.0 if sig_risk else 0.4,
                      linestyle='-' if sig_risk else ':')
        ax_path_a.plot(a_risk, +0.15, 's', color=COLOR_RISK, markersize=12,
                      markerfacecolor=COLOR_RISK if sig_risk else 'white',
                      markeredgewidth=2, alpha=1.0 if sig_risk else 0.4)
        ax_path_a.text(a_risk, +0.15 + 0.1, f"β={a_risk:.2f}", ha='center', va='bottom',
                       fontsize=12, color=COLOR_RISK, fontweight='bold', alpha=1.0 if sig_risk else 0.4)
        ...
        ax_path_b.set_xlabel(f"Path B Coef ({dim.upper()} → Delayed Mood)", fontweight='bold', fontsize=11)
        ax_path_b.set_title(f"Path B: {dim.upper()} → Delayed Mood", fontweight='bold', pad=12, fontsize=12)
        ...
        plt.tight_layout()
        out_path = os.path.join(output_dir, f"del_mood_mediation_{dim}.png")
        plt.savefig(out_path, dpi=300, bbox_inches="tight")
        plt.savefig(str(out_path).replace('.png', '.svg'), format='svg', bbox_inches="tight")
        plt.close()
        
    print(f"Individual delayed mood mediation figures saved in: {output_dir}")
...
def main() -> None:
    ...
    plot_individual_delayed_mood_mediation_figures(results, individual_dir)
